...
class TestMemoryOptimization:
    ...
    def test_logger_registry_size_limit(self):
        ...
        loggers = []
        for i in range(5):
            logger = LoggerFactory.get_or_create_logger(LoggerType.BASIC, name=f"test_logger_{i}", level=LogLevel.INFO)
    ...
    def test_logger_registry_ttl(self):
        ...
        # Create a logger
        logger1 = LoggerFactory.get_or_create_logger(LoggerType.BASIC, name="ttl_test_logger", level=LogLevel.INFO)
        ...
        logger2 = LoggerFactory.get_or_create_logger(LoggerType.BASIC, name="new_logger", level=LogLevel.INFO)
    ...
    def test_memory_stats_reporting(self):
        ...
        for i in range(3):
            LoggerFactory.get_or_create_logger(LoggerType.BASIC, name=f"stats_test_{i}", level=LogLevel.INFO)
    ...
    def test_weak_reference_tracking(self):
        ...
        def create_temporary_loggers():
            loggers = []
            for i in range(3):
                logger = BasicLog(name=f"weak_ref_test_{i}", level=LogLevel.INFO.value)
                loggers.append(logger)
            return len(loggers)
    ...
    def test_memory_leak_prevention(self):
        ...
        for batch in range(5):
            ...
            batch_loggers = []
            for i in range(10):
                logger = BasicLog(name=f"leak_test_batch_{batch}_logger_{i}", level=LogLevel.INFO.value)
    ...
    def test_logger_cleanup_on_context_exit(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with SizeRotatingLog(
                name="cleanup_test_size", directory=temp_dir, filenames=["test.log"], level="INFO"
            ) as logger2:
    ...
    def test_registry_memory_management_edge_cases(self):
        ...
        logger = LoggerFactory.get_or_create_logger(LoggerType.BASIC, name="edge_case_logger", level=LogLevel.INFO)
        ...
        for i in range(50):
            logger = LoggerFactory.get_or_create_logger(
                LoggerType.BASIC, name=f"large_limit_test_{i}", level=LogLevel.INFO
            )
            assert logger is not None
            ...
            try:
                for i in range(10):
                    formatter = get_cached_formatter(
                        f"Worker {worker_id} format {i}: %(message)s", f"%Y-%m-%d W{worker_id}I{i}"
                    )
    ...
    def test_memory_optimization_integration(self):
        ...
        for i in range(10):
            logger = LoggerFactory.get_or_create_logger(LoggerType.BASIC, name=f"integration_test_{i}")
