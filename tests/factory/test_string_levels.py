...
class TestStringLevels:
    ...
    def test_basic_logger_string_levels(self):
        ...
        for level_str, expected_level in test_cases:
            logger = BasicLog(name=f"test_basic_{level_str}", level=level_str)
    ...
    def test_mixed_enum_and_string_usage(self):
        ...
        logger_enum = BasicLog(name="enum_logger", level=LogLevel.DEBUG)

        # Create logger with string
        logger_string = BasicLog(name="string_logger", level="debug")
    ...
    def test_string_level_aliases(self):
        ...
        logger_warn = BasicLog(name="warn_test", level="warn")
        logger_warning = BasicLog(name="warning_test", level="warning")
        assert logger_warn.level == logger_warning.level == logging.WARNING

        # Test CRIT alias for CRITICAL
        logger_crit = BasicLog(name="crit_test", level="crit")
        logger_critical = BasicLog(name="critical_test", level="critical")
    ...
    def test_invalid_string_level_fallback(self):
        ...
        for invalid_level in invalid_levels:
            logger = BasicLog(name=f"invalid_{invalid_level or 'empty'}", level=invalid_level)
    ...
    def test_factory_registry_with_string_levels(self):
        ...
        logger1 = LoggerFactory.get_or_create_logger("basic", name="registry_test", level="info")
        assert logger1.level == logging.INFO

        # Get the same logger again (should be cached)
        logger2 = LoggerFactory.get_or_create_logger("basic", name="registry_test", level="debug")
    ...
    def test_comprehensive_string_level_functionality(self):
        ...
        basic = BasicLog(name="comp_basic", level="debug")

        size_rotating = SizeRotatingLog(
            name="comp_size",
            level="info",
            directory=self.temp_dir,
        ...
        timed_rotating = TimedRotatingLog(
            name="comp_timed",
            level="warning",
            directory=self.temp_dir,
