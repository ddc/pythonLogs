...
from pythonLogs import (
    LoggerFactory,
    LoggerType,
    LogLevel, clear_logger_registry,
    BasicLog,
    SizeRotatingLog,
    TimedRotatingLog,
)
...
class TestThreadSafety:
    ...
    def test_concurrent_logger_factory_creation(self):
        ...
        def create_logger_worker():
            ...
            _logger = LoggerFactory.get_or_create_logger(
                LoggerType.BASIC, 
                name=logger_name,
                level=LogLevel.INFO
            )
    ...
    def test_registry_clear_during_operations(self):
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=num_worker_threads + 1) as executor:
                ...
                worker_futures = [
                    executor.submit(continuous_worker, i) for i in range(num_worker_threads)
                ]
